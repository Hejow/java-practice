...
public class _2206 {
	static class Point {
		...
		static Point never(int x, int y, int distance) {
			return new Point(x, y, distance, true);
		}

		static Point broke(int x, int y, int distance) {
			return new Point(x, y, distance, false);
		}
	}
			...
			for (int i = 0; i < 4; i++) {
				...
				if (graph[nx][ny] == WALL) {
					if (point.neverBroke) {
						...
						queue.add(Point.broke(nx, ny, nDistance));
					}
				} else {
					if (point.neverBroke) {
						if (!visited[nx][ny][0]) {
							visited[nx][ny][0] = true;
							queue.add(Point.never(nx, ny, nDistance));
						}
					} else {
						if (!visited[nx][ny][1]) {
							visited[nx][ny][1] = true;
